...

{
    ...
    //initializeEnv(env1, perimeter1,perimeter2,envCfgFilename, motPrimFilename);
    
    initializeEnv(env1, perimeter1,envCfgFilename, motPrimFilename);
    
    //Do not intialise another environment!
    //initializeEnv(env2, perimeter2, envCfgFilename, motPrimFilename);
 
    // specify a start and goal state
    int start_id, goal_id;
    //setEnvStartGoal(env1, .11, .11, 0, 35, 47.5, 0, start_id, goal_id); //Start x,y,theta and goal x,y,theta
    setEnvStartGoal(env1, .11, .11, 0, 0.35, 0.3, 0, start_id, goal_id);
