...
void initializeEnv(EnvironmentNAVXYTHETALAT& env, //the env file
                   vector<sbpl_2Dpt_t>& perimeter, //the perimeter
...
void planxythetalat(char* envCfgFilename, char* motPrimFilename){
    ...
    createFootprint(perimeter); //Passed perimeter by reference
    ...
    setEnvStartGoal(env, .11, .11, 0, 0.35, 0.3, 0, start_id, goal_id);
